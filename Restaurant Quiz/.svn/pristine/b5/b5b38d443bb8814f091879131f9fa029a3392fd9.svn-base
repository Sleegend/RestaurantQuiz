	...
	private ItemizedIconOverlay<OverlayItem> createBusLocnOverlay() {
		...
		OnItemGestureListener<OverlayItem> gestureListener = new OnItemGestureListener<OverlayItem>() {
			...
			public boolean onItemSingleTapUp(int index, OverlayItem oi) {
				...
				AlertDialog dlg = createSimpleDialog(oi.getTitle(),
						oi.getSnippet());
				dlg.show();
				if (selectedBus != null) {
				...
				selectedBus = oi;
				selectedBus.setMarker(getResources()
						.getDrawable(R.drawable.selected_bus));
				routeOverlays.clear();
				retrieveBusRoute(selectedStop.getBusLocations().get(index));
				Log.d(LOG_TAG, "addroute routeOverlay size: " + routeOverlays.size());
				// addBusRoute before add others
//				om.clear();
//				Log.d(LOG_TAG, "addroute clear: " + om.size());
//				
//				
//				om.addAll(routeOverlays);
//				Log.d(LOG_TAG, "addroute addroute: " + om.size());
//				
//				om.add(busStopLocationOverlay);
//				Log.d(LOG_TAG, "addroute addstop: " + om.size());
//				om.add(busLocnOverlay);
//				Log.d(LOG_TAG, "addroute addbus: " + om.size());
//				om.add(legendOverlay);
//			
		...
		return new ItemizedIconOverlay<OverlayItem>(
				new ArrayList<OverlayItem>(), 
				        getResources().getDrawable(R.drawable.map_pin_blue),  // selected_bus
				        gestureListener, rp);
	}
	...
	private void plotBuses(boolean zoomToFit) {
		...
		for (BusLocation next : busLocations){
			double lat = next.getLatLon().getLatitude();
			double lon = next.getLatLon().getLongitude();
		...
		if (busLocations.size() > 0) {
			...
			if (zoomToFit){
			int latSpanE6 = (int) Math.round((Math.abs(maxLat - minLat) * 1E6));
			int lonSpanE6 = (int) Math.round((Math.abs(maxLon - minLon) * 1E6));
	...
	private void retrieveBusRoute(BusLocation bus) {
		if (bus.getRoute().hasSegments()){
			...
			Log.d(LOG_TAG, "bus Segments" + bus.getRoute().getSegments().size());
		}
		else{
			new GetRouteInfo().execute(bus.getRoute());
			Log.d(LOG_TAG, "bus Segments add new" + bus.getRoute().getSegments().size());
		}
	}
	...
	private void plotBusRoute(BusRoute rte) {
		if (rte == null){
			routeOverlays.clear();
			mapView.postInvalidate();
			Log.d(LOG_TAG, "rte is null" + routeOverlays.size());
		}
			...
			if (segments.size() > 0) {
				...
				for (int i = 0; i < segments.size(); i++) {
					...
					for (LatLon latlon : next.getPoints()) {
	                    GeoPoint gPt = new GeoPoint(latlon.getLatitude(),
	                                    latlon.getLongitude());
	                    po.addPoint(gPt);
					}
					...
					Log.d(LOG_TAG, "rte add po" + routeOverlays.size());
				}
