...
public class ScoreTracker {
    ...
    public void addScore(Score s) {
        if (instance.getScores().size() >= COUNT) {
            instance.getScores().remove(0);
            instance.getScores().add(s);
            highPointsEarned = findHighestPointsEarned();
            minAttempts = findMinAttemptsTaken();
    ...
    private int findHighestPointsEarned() {
        ...
        int highestPointsEarned = scores.get(0).getPointsEarned();

        for (Score score : scores) {
            if (score.getPointsEarned() >= highestPointsEarned) {
                highestPointsEarned = score.getPointsEarned();
            }
        }
    ...
    private int findMinAttemptsTaken() {
            ...
            int minAttemptsTaken = scores.get(0).getNumAttempts();

            for (Score score : scores) {
                if (score.getNumAttempts() <= minAttemptsTaken) {
                    minAttemptsTaken = score.getNumAttempts();
                }
            }
            return minAttemptsTaken;
        }
